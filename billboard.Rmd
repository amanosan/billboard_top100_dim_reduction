...
library(knitr)
knitr::opts_chunk$set(cache = T, cache.lazy = F, warning = F, 
...
Let's use dimensionality reduction on the Billboard Top 100 songs connecting data about where the songs were in the rankings with mostly audio features available from Spotify.
We will see which features are important in keeping the songs for the most time on the Top100 charts. The features are taken from Spotify (in the audio_features.csv file).
...
`tidy()` the recipe to find features that contribute the most to the Principal Components:
```{r}
rec_trained %>% 
    step_pca(all_numeric_predictors(), num_comp = 4) %>% 
    prep() %>% 
